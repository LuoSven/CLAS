    ...
    public class Hocy_Hook
    {
		...
		public bool InstallHook(string flagsinfo)
		{
           ...
           // Assembly.GetExecutingAssembly().GetModules()[0]
            ...
            //}
		    int s = 0;
		    var id = Win32API.GetWindowThreadProcessId(pInstance, out s);
			if ( this.m_pKeyboardHook == IntPtr.Zero )
			{
				this.m_KeyboardHookProcedure = new HookProc( this.KeyboardHookProc );
				this.m_pKeyboardHook = Win32API.SetWindowsHookEx( WH_Codes.WH_KEYBOARD_LL,	this.m_KeyboardHookProcedure, pInstance,s );
				if ( this.m_pKeyboardHook == IntPtr.Zero )
				{
					this.UnInstallHook();
					return false;
